...
class TDMPC():
	...
	def update(self, replay_buffer, step):
		...
		obs, next_obses, action, reward, idxs, weights = replay_buffer.sample() # Sample Traj.
		self.optim.zero_grad(set_to_none=True)
		self.std = h.linear_schedule(self.cfg.std_schedule, step)
		self.model.train() # start training 

		# Representation
		z = self.model.h(self.aug(obs)) # Encode first observation
		zs = [z.detach()]

		consistency_loss, reward_loss, value_loss, priority_loss = 0, 0, 0, 0 # Initiate J for loss accumulation
		...
		self.model.eval() # turn off training 
